...
export function InvestmentModal({ open, setOpen, method,data }) {
  ...
  const handleDeposit = (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      method === "bitcoin"
        ? navigate(`/dashboard/bitcoin/gateway/${data.id}/detail?amount=${amount}`)
        : navigate(`/dashboard/ethereum/gateway/${data.id}/detail?amount=${amount}`);
    } else {
